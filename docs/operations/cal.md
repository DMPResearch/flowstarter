...
### Two traps that only a real booking finds

Both were found by making an actual booking on a provisioned page and watching
for the row that never arrived. Neither is visible to a unit test, and both
look exactly like success from the client's side.

**1. Cal Video is not seeded, and its failure eats the webhook.** An event type
with no `locations` falls back to Cal Video, whose app needs a Daily API key
this instance does not have. The booking is still created and the page still
says "This meeting is scheduled" — but `EventManager.create` throws
`Location app daily-video is either disabled or not seeded at all` first, and
the `BOOKING_CREATED` webhook is never sent. So the provisioner sets a location
that needs no third-party app; `phone` (the attendee gives a number) is the
default, and `CAL_EVENT_LOCATION` takes `attendeeInPerson` or `none` instead.

A related finding, for anyone tempted to add booking questions: writing a
partial `bookingFields` array **freezes Cal's system fields**. Cal merges what
it finds with its own defaults, so the phone field the location then required
came back hidden and no booking could be completed. The provisioner leaves
`bookingFields` null and takes Cal's own set — name, email, phone, notes,
guests — which is what the site needs anyway.

**2. The webhook endpoint was never reachable.** `/api/integrations/cal/…` was
not in `PUBLIC_ROUTES`, so Clerk's middleware answered every delivery with
`401 Authentication required` before the route's signature check ran. From
Cal's side that is indistinguishable from a wrong URL. It has been public since
2026-09-13, with the same reasoning as the Stripe webhook and the lead-capture
endpoint: the caller is a server with no session, and it authenticates itself
with an HMAC inside the route. `src/__tests__/route-manifest.test.ts` now pins
all three.
...
plain AGPL self-host.

`TASKER_ENABLE_WEBHOOKS` is unset, and that matters more than it looks.
Cal's `sendOrSchedulePayload` sends a booking webhook inline when it is
unset and queues it on the Tasker when it is `1` — and the Tasker is drained
by `/api/tasks/cron`, which nothing on this box calls. Set it and every
booking would silently stop reaching the client's dashboard. Email goes out through Resend's SMTP endpoint with the
...
git. Restores go through `restore.sh`, which now takes a container name
(`--database flowstarter-cal-db`) as well as a Supabase project id.

Two things the first real run found, neither of which the stubbed tests could:

- The Supabase container reports `POSTGRES_USER=supabase_admin`, a role the
  socket asks for a password for. Discovering the role from the container's own
  environment — correct for Cal, whose role is `calcom` — turned a working
  Supabase dump into `fe_sendauth: no password supplied` and a **zero-byte
  file** sitting in the backup directory looking like the others. Supabase is
  now an explicit exception, dumped as `postgres` the way it always was.
- A failing `pg_dump` left that zero-byte file behind. It is deleted now, and
  every dump passes `-w` so a nightly timer can never sit on a password prompt
  nobody will answer.

### Host state changed on 2026-09-13

Everything here is on `fs-sites-01` and none of it is in git, so it is written
down rather than remembered:

| What          | Where                                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DNS           | `cal.flowstarter.dev` A `178.104.218.87`, dns-only, TTL 60, record id `401c3c48bdd20f48c5878e7b17172558`. **New record. Nothing existing in either zone was read-modified or deleted.**                                                                                                                                                                                          |
| Secrets       | `/etc/flowstarter/cal.env`, root, 600 — Postgres password, `NEXTAUTH_SECRET`, `CALENDSO_ENCRYPTION_KEY`, the Resend SMTP password, the admin account and the provisioner role's password, all generated on the box                                                                                                                                                               |
| App env       | a marked `# --- Self-hosted Cal.com ---` block in `staging.env` (filled in) and `prod.env` (deliberately empty)                                                                                                                                                                                                                                                                  |
| Cal admin     | one user, `darius`, role `ADMIN`, created through `/api/auth/setup`. The password is in `cal.env` and nowhere else; Darius should sign in and change it                                                                                                                                                                                                                          |
| Postgres role | `flowstarter_provisioner`, the grants above                                                                                                                                                                                                                                                                                                                                      |
| Backups       | **the nightly timer had never been installed on this host.** `flowstarter-backup.timer` is now enabled (03:00 UTC, up to 30 minutes of jitter), with `/etc/flowstarter/backup.env` and a generated `backup-gpg-passphrase` (600) beside it. First run verified by hand: both database dumps, the sites tarball, the encrypted `/etc/flowstarter` tarball and a `manifest.sha256` |

The image and the Cal source checkout on the box (`/opt/flowstarter/cal/src`,
pinned to `v6.2.0`) take about 8.5 GB of the 150 GB disk. The source is only
needed to read Cal's own code; deleting it costs nothing but a `git clone`.
