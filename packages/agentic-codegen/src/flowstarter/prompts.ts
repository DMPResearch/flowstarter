...
- When the spec includes assetLibrary, it honestly describes every shipped artwork file; use it to choose fitting art for each slot. Prefer entries whose "kind" shows real work — "ui-project", then "photo-generic" — for project and case-study cards: those read as a portfolio. Entries marked "abstract" are decorative fallback art for a project, case-study or journal card only, and only as a last resort when no real-work asset and no client photo fits; a grid of abstract shapes makes a finished site look unfinished. An entry marked "kind":"photo-person" depicts the template's demo persona: never present it as the client. Fill that slot with a cachedAssets photo of the client when one exists; otherwise choose a non-person asset from the library.
