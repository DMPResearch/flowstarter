...
/**
 * The code a build fails with when a portrait ended up somewhere it may not
 * be. Its own code rather than `PLACEHOLDER_IMAGE_SHIPPED`, because the two
 * defects have opposite remedies: that one means the site is wearing art that
 * is not the client's and it should come out, this one means the site is
 * holding the client's own face and has put it in the wrong place.
 */
export const PORTRAIT_MISPLACED = 'PORTRAIT_MISPLACED';
...
/**
 * The findings, listed, shared by both phrasings below.
 *
 * Split into a record and a brief for the reason `placeholder-images.ts` gives
 * for its own pair: the repair brief names the exact files an agent may
 * rewrite, and the failure record must not read as an instruction. A build
 * that failed is being explained to a person; a build that is being repaired
 * is being told what to do. The same sentence cannot do both without one of
 * the two readers being misled.
 */
function listFindings(
  ...
  const overflow =
    findings.length > listed.length
      ...
      : '';
  return `${detail}${overflow}`;
}

function findingCodes(findings: readonly PortraitSlotFinding[]): string {
  return Array.from(
...
/**
 * The failure record: what is wrong and why it cannot stand. No instructions,
 * because this is what a person reads on a build that did not ship.
 */
export function describePortraitSlotIssue(
  findings: readonly PortraitSlotFinding[],
  portrait: BuildPortrait,
): string {
  const allowed = allowedPlacements(portrait.verdict).join(', ');
  return (
    `${findingCodes(findings)}: the client’s photograph is ` +
    `${portrait.publicPath}, ${portrait.longEdge}px on its longest side, ` +
    `which the size rule allows only as: ${allowed}. ` +
    `${listFindings(findings, portrait)}.`
  );
}

/**
 * The repair brief: what to do about it, naming the file to use.
 *
 * Says "at its own size or smaller, never scaled up" every time, because the
 * defect this gate exists for is an avatar-sized photograph stretched to fill
 * a hero, and an agent told only to "move the picture" will cheerfully move it
 * into another slot that is also too big for it.
 */
export function describePortraitSlotRepair(
  findings: readonly PortraitSlotFinding[],
  portrait: BuildPortrait,
): string {
  const allowed = allowedPlacements(portrait.verdict).join(', ');
  return (
    `The client’s own photograph is ${portrait.publicPath}, ` +
    `${portrait.longEdge}px on its longest side, and may be used only as: ` +
    `${allowed}. Put it in the ${preferredPlacement(portrait.verdict)} slot ` +
    'at its own size or smaller, never scaled up, and where it does not fit ' +
    'render initials or no image rather than the template’s stock art. ' +
    `Fix: ${listFindings(findings, portrait)}.`
  );
}
...
export function findPortraitSlotIssue(
  ...
  return describePortraitSlotIssue(findings, portrait);
}
