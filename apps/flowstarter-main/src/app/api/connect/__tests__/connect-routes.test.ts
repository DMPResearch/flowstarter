...
/**
 * The token the stubbed provider hands back.
 *
 * Minted per run rather than written down, for the same reason the provider
 * credentials are: a quoted value at a token-shaped key is what a secret
 * scanner is built to find, and a fixture that merely looks like a credential
 * still costs somebody a triage. See `portrait-test-credentials.ts`.
 */
const PROVIDER_TOKEN = testCredential('provider-access');
...
function stubProviderFetch(profile: Record<string, unknown>): void {
  global.fetch = vi.fn(async (_input: unknown, init?: { method?: string }) => {
    const body =
      init?.method === 'POST' ? { access_token: PROVIDER_TOKEN } : profile;
    return {
      ok: true,
      status: 200,
      json: async () => body,
