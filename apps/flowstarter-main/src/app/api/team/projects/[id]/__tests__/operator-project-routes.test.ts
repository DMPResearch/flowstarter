...
/**
 * Every operator route under `/api/team/projects/[id]`, through the REAL
...
function everyOperatorRoute(): RouteCase[] {
  const base = `/api/team/projects/${WORKSPACE}`;
  return [
    [
      'ai/generate-copy',
      () =>
...
describe('a workspace that is not yours to operate', () => {
  it('is 403 on every /api/team/projects/[id] route, and nothing of it is read', async () => {
