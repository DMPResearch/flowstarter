 ...
 *     and where the meeting happens. They read config, never a network.
...
export interface CalProvisioningConfig {
  ...
  /** Where the meeting happens. See `calEventLocations` for why it matters. */
  eventLocation: CalEventLocation;
}
...
/**
 * One of the three locations this product knows how to set up without asking
 * anybody for a credential. An unrecognised value falls back rather than
 * reaching Postgres: a typo in an env var must not produce an event type whose
 * bookings all fail inside Cal.
 */
function envEventLocation(raw: string | undefined): CalEventLocation {
  const value = raw?.trim();
  return value === 'attendeeInPerson' || value === 'none' || value === 'phone'
    ? value
    : DEFAULTS.eventLocation;
}
...
export function calProvisioningConfig(
  ...
  return {
    ...
    eventLocation: envEventLocation(env.CAL_EVENT_LOCATION),
  };
}
...
/**
 * Where the meeting happens, as Cal stores it on the event type.
 *
 * THIS IS NOT COSMETIC, AND THE DEFAULT IS NOT CAL'S. Leave `locations` unset
 * and Cal falls back to Cal Video, whose app is not seeded on a self-host
 * without a Daily API key. Every booking then fails inside `EventManager`
 * with "Location app daily-video is either disabled or not seeded at all" —
 * and, because that failure happens before the trigger fires, **the
 * BOOKING_CREATED webhook is never sent**. The client's booking page looks
 * like it worked, the meeting exists in Cal, and nothing ever reaches their
 * dashboard. Measured on v6.2.0, 2026-09-13.
 *
 * So a location that needs no third-party app is chosen for them:
 *
 *   phone             the attendee gives a number and the business rings
 *                     them. The default, because it is what a service
 *                     business's intro call actually is, and it asks the
 *                     visitor for something they expect to give.
 *   attendeeInPerson  the attendee gives an address, for a trade that
 *                     travels to the job.
 *   none              no location at all. Cal's booker then offers Cal Video
 *                     again, so this is only for an instance that has one.
 *
 * The client can change it in Cal afterwards; this is the starting point, not
 * a policy.
 */
export type CalEventLocation = 'phone' | 'attendeeInPerson' | 'none';

export function calEventLocations(config: CalProvisioningConfig): Json {
  if (config.eventLocation === 'none') return [] as unknown as Json;
  return [{ type: config.eventLocation }] as unknown as Json;
}
...
export async function provisionWorkspaceCalendar(
  ...
  try {
    ...
    const calEventTypeId = await client.ensureEventType(calUserId, {
      ...
      scheduleId,
      locations: calEventLocations(config),
    });
    const secret =
