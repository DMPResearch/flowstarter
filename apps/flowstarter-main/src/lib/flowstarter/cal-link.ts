...
export function calLinkRejectionMessage(reason: CalLinkRejection): string {
  switch (reason) {
    ...
    case 'host':
      // Deliberately does not name the platform's own host: an environment
      // with no self-hosted instance would be offering a link that 404s.
      return 'That is not a calendar we can embed. Use your cal.com link, or the booking page we made you.';
    case 'reserved':
