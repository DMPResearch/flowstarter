...
/**
 * A response of the shape `lib/net/safe-fetch` reads.
 *
 * Three things changed when the download moved onto the shared adapter, and
 * each of them is a way this double could have quietly stopped testing the
 * real thing:
 *
 *   `status`      the adapter judges the code itself; the old `ok` flag is
 *                 not something it looks at.
 *   content type  an image fetch refuses a body that does not say it is an
 *                 image. A double with no type would be exercising a laxer
 *                 adapter than production runs.
 *   a real stream the cap is enforced as the body arrives. Handing back only
 *                 `arrayBuffer()` takes the adapter's fallback path, where the
 *                 whole body is allocated first, so the over-the-cap case
 *                 would pass for the wrong reason.
 *
 * `contentType` is overridable so one test can prove the type is actually
 * checked; it defaults to a real image type because every other test here is
 * about the bytes rather than the header.
 */
function responding(
  body: Buffer,
  init: {
...
describe('capturePortraitFromProvider', () => {
  ...
  it('refuses a URL that answers with something that is not a picture at all', async () => {
    // The bytes are never read on this path. `lib/net/safe-fetch` is told to
    // expect an image and refuses the response on its declared type, which is
    // what stops a provider URL that has been pointed at a log file from being
    // downloaded in full before anybody looks at it. `not_public_url` rather
    // than `not_an_image` on purpose: we declined to take the body, we did
    // not take it and find it wanting.
    const fake = fakeSupabase();
    expect(
      await capturePortraitFromProvider({
        connectionId: CONNECTION,
        ...
        previewId: PREVIEW,
        workspaceId: null,
        fetchImpl: serving(Buffer.from('not a picture'), {
    ...
    ).toMatchObject({ status: 'skipped', reason: 'not_public_url' });
    expect(storeFunnelAsset).not.toHaveBeenCalled();
  });
