...
/**
 * The two absences this suite pins, as named constants rather than quoted
 * empties at a secret-shaped or token-shaped key. Nothing in a committed file
 * should read as though a credential were being assigned, even when the value
 * is nothing at all. See `portrait-test-credentials.ts`.
 */
...
describe('portraitAuthorizeUrl', () => {
  ...
  it('sends LinkedIn every parameter the OpenID Connect flow needs', () => {
    ...
    expect(`${url.origin}${url.pathname}`).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.linkedin.authorizeUrl
    );
  ...
  it('sends Instagram the business-basic scope and nothing else', () => {
    ...
    expect(`${url.origin}${url.pathname}`).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.instagram.authorizeUrl
    );
    expect(url.searchParams.get('response_type')).toBe('code');
...
describe('exchangePortraitCode', () => {
  const REDIRECT_URI =
    'https://app.flowstarter.dev/api/connect/linkedin/callback';
  // Minted per run. The point of this constant is that it must never appear
  // in a returned profile, and a value that is different on every run proves
  // that by identity rather than by a string somebody could copy.
  const TOKEN = testCredential('provider-access');
  ...
  it('exchanges a LinkedIn code as a form post and reads userinfo with a bearer token', async () => {
    ...
    expect(tokenCall.url).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.linkedin.exchangeUrl
    );
    ...
    expect(profileCall.url).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.linkedin.profileUrl
    );
  ...
  it('reads the Instagram profile with the fields list and the token on the query string', async () => {
    ...
    expect(tokenCall.url).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.instagram.exchangeUrl
    );
    ...
    expect(`${profileUrl.origin}${profileUrl.pathname}`).toBe(
      PORTRAIT_PROVIDER_ENDPOINTS.instagram.profileUrl
    );
