...
/** What a real environment would hold, for the cases that read process.env. */
const LIVE_ID = testCredential('linkedin-live-client-id');
const LIVE_SECRET = testCredential('linkedin-live-client');
...
describe('portraitProviderCredentials', () => {
  ...
  it('reads the live environment when no environment is passed', () => {
    vi.stubEnv('LINKEDIN_CLIENT_ID', LIVE_ID);
    vi.stubEnv('LINKEDIN_CLIENT_SECRET', LIVE_SECRET);
    expect(portraitProviderCredentials('linkedin')).toEqual({
      provider: 'linkedin',
      clientId: LIVE_ID,
