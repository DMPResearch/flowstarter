...
/**
 * A response that says what it is.
 *
 * The content type is not decoration here. `lib/net/safe-fetch` refuses a
 * body whose type is not an image when the caller asked for one, which is what
 * stops a public page from pointing us at a log file, and a double that
 * answered with no type at all would be testing against a laxer adapter than
 * production runs. Buffers are pictures and strings are pages, which is what
 * every caller in this suite means anyway.
 */
function responding(status: number, body: Buffer | string): Response {
  const isPage = typeof body === 'string';
  const bytes = isPage ? Buffer.from(body, 'utf-8') : body;
  ...
  return {
    status,
    headers: new Headers({
      'content-type': isPage ? 'text/html; charset=utf-8' : 'image/png',
    }),
    body: stream,
...
/** A response that answers with a type the image rule will not take. */
function respondingAsText(status: number, body: Buffer): Response {
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
...
describe('readAutomaticPortraitSources', () => {
  ...
  it('leaves a source unmeasured when the URL answers with something that is not a picture', async () => {
    // A GitHub avatar URL that answers `text/plain` is not an avatar,
    // whatever the path says. The shared adapter refuses it on the declared
    // type before the body is read, so the source comes back with a URL and no
    // dimensions, and the rule turns that into `size_unknown` rather than a
    // placement. We do not place what we have not measured.
    const { fetchImpl } = router({
      ...ALL_THREE,
      [GITHUB_AVATAR]: () => respondingAsText(200, png(460, 460)),
    });
    const observations = await readAutomaticPortraitSources({
      urls: LINKS,
      fullName: 'Darius Popescu',
    ...
    expect(observations.github).toEqual({
      handle: 'darius',
      picture: { url: GITHUB_AVATAR, width: null, height: null },
    });
  });
