...
describe('calProvisioningConfig', () => {
  ...
  it('takes every knob from the environment', () => {
    const parsed = calProvisioningConfig({
      ...
      CAL_EVENT_LOCATION: 'attendeeInPerson',
    });
    ...
    expect(parsed.eventLocation).toBe('attendeeInPerson');
  });
...
describe('timezone and locale', () => {
  ...
  it('gives the event type a location that needs no third-party app', () => {
    // Cal falls back to Cal Video when this is unset, and Cal Video is not
    // seeded on a self-host without a Daily key: every booking then fails
    // inside EventManager BEFORE the BOOKING_CREATED webhook fires, so the
    // client's page looks like it worked and their dashboard stays empty.
    expect(calEventLocations(config())).toEqual([{ type: 'phone' }]);
    expect(
      calEventLocations(config({ eventLocation: 'attendeeInPerson' }))
    ).toEqual([{ type: 'attendeeInPerson' }]);
    expect(calEventLocations(config({ eventLocation: 'none' }))).toEqual([]);
  });

  it('falls back to the phone location when the variable is nonsense', () => {
    expect(
      calProvisioningConfig({ CAL_EVENT_LOCATION: 'zoom' }).eventLocation
    ).toBe('phone');
  });
});
...
describe('the Postgres client', () => {
  ...
  it('creates the event type with its location, and the join row Cal resolves bookings through', async () => {
    const { issued, run } = recording({
