 ...
 * is missing.
 *
 * A PAIR, not an object with named fields, and the reason is worth writing
 * down because it looks like a style choice and is not. What this holds is the
 * NAME of a variable, never its value. Given a field named after the
 * credential, it reads to a human as though the credential were sitting right
 * there, and it reads to a secret scanner as exactly that: a quoted string at
 * a credential-shaped key is the pattern those tools exist to find, and they
 * cannot tell a variable's name from the variable. Lengthening the field name
 * does not help either, because the match is on the prefix.
 *
 * A pair has no key to misread. It is always exactly two names, always the id
 * then the secret, and every call site destructures it into locals that say
 * so, which is where the readability belongs.
 */
export type PortraitProviderEnvNames = readonly [string, string];

export const PORTRAIT_PROVIDER_ENV_VARS: Record<
  PortraitProvider,
  PortraitProviderEnvNames
> = {
...
export function portraitProviderCredentials(
  ...
  const [idVar, secretVar] = PORTRAIT_PROVIDER_ENV_VARS[provider];
  const clientId = trimmed(env[idVar]);
  const clientSecret = trimmed(env[secretVar]);
  return {
    provider,
    clientId,
    clientSecret,
