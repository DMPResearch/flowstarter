...
async function measurePicture(
  ...
  // A syntactic pre-check, and explicitly not the security boundary: it drops
  // a URL that can never succeed without spending a DNS question on it. The
  // destination is decided inside `fetchPublicResource`, after resolution,
  // against the address. See the note on `isFetchableUrl` itself.
  if (!isFetchableUrl(url)) return unmeasured;
...
function ogImageFrom(html: string, baseUrl: string): string | null {
  ...
  return isFetchableUrl(absolute) ? absolute : null;
}
...
export async function downloadPortraitPicture(input: {
  ...
  if (!isFetchableUrl(input.url)) return null;
