...
/**
 * Why the shared adapter refused, in this module's vocabulary.
 *
 * Four reasons in, four reasons out, and the mapping is written down rather
 * than collapsed to "unreadable" because the client is shown a sentence about
 * it. `blocked` is the one worth naming apart: it means we declined to make
 * the request at all, not that the request failed.
 */
const CAPTURE_REASON: Record<PublicFetchFailure, PortraitCaptureFailure> = {
  blocked: 'not_public_url',
  too_large: 'too_large',
  timeout: 'unreadable',
  network_error: 'unreadable',
};

/**
 * The bytes behind a provider's picture URL.
 *
 * This used to be a hand-rolled fetch with `redirect: 'follow'` and an
 * `arrayBuffer()`, which is two bugs wearing the shape of a download: the
 * runtime chased every hop without anybody looking at where hop two went, and
 * the cap was enforced after the whole body had already been allocated. #135
 * built one adapter that gets both right, and every other caller in the
 * product now goes through it. This is the last one; a fork left behind here
 * would be the third copy of "is this safe to fetch" that #135 existed to
 * delete.
 *
 * `expect: 'image'` is the portrait-specific part, and it is an option rather
 * than a fork: a URL a provider handed us that answers with a 400 MB log file
 * is refused on the content type before the body is read.
 */
async function downloadPicture(input: {
  ...
  const outcome = await fetchPublicResource({
    ...
    expect: 'image',
    fetchImpl: input.fetchImpl as unknown as FetchLike,
  });
  if (outcome.status !== 'ok') {
    return { ok: false, reason: CAPTURE_REASON[outcome.reason] };
  }
...
export async function capturePortraitFromProvider(input: {
  ...
  if (!profile.pictureUrl || !isFetchableUrl(profile.pictureUrl)) {
