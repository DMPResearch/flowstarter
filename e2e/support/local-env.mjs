...
const ENV_FILES = [
  ...
  // Generated by `pnpm db:env` (scripts/supabase-local-env.mjs) from
  // `supabase status`. Last, so it wins: it is the freshest read of the
  // local stack's actual URL and keys, which rotate whenever `supabase
  // start` reinitializes.
  'apps/flowstarter-main/.env.development.local',
];

/** The app's env files merged, later files winning. Missing files are skipped. */
